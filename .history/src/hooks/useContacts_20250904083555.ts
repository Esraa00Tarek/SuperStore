...
export const useContacts = () => {
  ...
  const addContact = async (contact: Omit<Contact, 'id' | 'createdAt' | 'status'>) => {
    try {
      const newContact = await contactsCollection.add({
        ...contact,
        createdAt: new Date().toISOString()
      });
      // Add 'status' property to the returned contact object
      const contactWithStatus = { ...newContact, status: 'new' as Contact['status'] };
