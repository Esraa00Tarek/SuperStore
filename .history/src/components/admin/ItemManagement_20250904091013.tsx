    ...
    <div className="space-y-4">
      ...
      <Dialog open={isViewing} onOpenChange={setIsViewing}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t('Item Details') || 'Item Details'}</DialogTitle>
          </DialogHeader>
          ...
          <div className="md:hidden p-4">
            {items.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {items.map((item, idx) => (
                  <div key={`${item.id}-${idx}`} className="category-card h-44 flex flex-col justify-between">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{categoryOptions.find(opt => opt.key === item.category)?.label || item.category}</p>
                        <p className="text-sm font-semibold mt-2">{item.priceValue} {item.priceCurrency}</p>
                      </div>
                    </div>
                    <div className="flex items-center justify-end gap-2">
                      <Button variant="ghost" size="icon" onClick={() => handleViewDetails(item)} title={t('View Details') || 'View Details'}>
                        <Eye className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleEdit(item)} title={t('Edit') || 'Edit'}>
                        <Edit className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(item.id)} title={t('Delete') || 'Delete'}>
              ...
              <div className="text-center text-sm text-muted-foreground">{t('No Items Found') || 'No Items Found'}</div>
            )}
          </div>
