            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <p><span className="font-medium text-gray-600">{t('admin.name') || 'Name'}:</span> {viewingItem.name}</p>
                    {viewingItem.nameAr && (
                      <p className={!viewingItem.nameAr ? 'hidden' : ''}>
                        <span className="font-medium text-gray-600">{t('admin.nameAr') || 'Name (Arabic)'}:</span> {viewingItem.nameAr}
                      </p>
                    )}
                    <p><span className="font-medium text-gray-600">{t('admin.price') || 'Price'}:</span> {viewingItem.priceValue} {viewingItem.priceCurrency}</p>
                    {viewingItem.category && (
                      <p className={!viewingItem.category ? 'hidden' : ''}>
                        <span className="font-medium text-gray-600">{t('admin.category') || 'Category'}:</span>{" "}
                        {categoryOptions.find(opt => opt.key === viewingItem.category)?.label || viewingItem.category}
                      </p>
                    )}
                    {viewingItem.seller && (
                      <p className={!viewingItem.seller ? 'hidden' : ''}>
                        <span className="font-medium text-gray-600">{t('admin.seller') || 'Seller'}:</span> {viewingItem.seller}
                      </p>
                    )}
                    <p><span className="font-medium text-gray-600">{t('admin.rating') || 'Rating'}:</span> {viewingItem.rating || 'N/A'}</p>
                  </div>
                </div>
                ...
                {viewingItem.image && (
                  <div className="space-y-2">
                    <h3 className="font-medium text-lg">{t('admin.image') || 'Image'}</h3>
                    <div 
              ...
              {(viewingItem.firstDescription || viewingItem.secondDescription || viewingItem.description) && (
                <div className="space-y-4 pt-4 border-t border-gray-200">
                  <h3 className="font-medium text-lg">{t('admin.description', 'Description')}</h3>
              ...
              <div className="pt-4 border-t border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-500">
                  {viewingItem.createdAt && (
                    <p><span className="font-medium text-gray-600">{t('admin.createdAt') || 'Created'}:</span> {formatDate(viewingItem.createdAt)}</p>
                  )}
                  {viewingItem.updatedAt && viewingItem.updatedAt !== viewingItem.createdAt && (
                    <p><span className="font-medium text-gray-600">{t('admin.updatedAt') || 'Updated'}:</span> {formatDate(viewingItem.updatedAt)}</p>
                  )}
                </div>
              </div>
            </div>
