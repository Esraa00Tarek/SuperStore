...
const AdminDashboard = ({ children }: AdminDashboardProps) => {
  ...
  const [initStatus, setInitStatus] = useState<{ success?: boolean; message?: string; error?: any } | null>(null);
  const navigate = useNavigate();
  const location = useLocation();

  // Navigation items
  const navItems = [
    { name: t('dashboard.overview') || 'Overview', icon: Home, path: '/admin/dashboard' },
