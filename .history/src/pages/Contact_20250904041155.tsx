...
const Contact = () => {
  ...
  const { contacts, loading, error: contactsError } = useContacts();

  useEffect(() => {
    if (!loading && !contactsError && contacts.length > 0) {
  ...
  }, [contacts, loading, contactsError]);
  ...
  const contactInfoItems = [
    {
      ...
      value: contactInfo.phone || (isRTL ? 'غير متوفر' : 'Not available'),
      action: () => contactInfo.phone ? window.location.href = `tel:${contactInfo.phone.replace(/\D/g, '')}` : {}
    },
    {
      icon: Mail,
      title: isRTL ? 'البريد الإلكتروني' : 'Email',
      value: contactInfo.email || (isRTL ? 'غير متوفر' : 'Not available'),
      action: () => contactInfo.email ? window.location.href = `mailto:${contactInfo.email}` : {}
    },
    {
      icon: MapPin,
      title: isRTL ? 'العنوان' : 'Address',
      value: isRTL 
        ? contactInfo.addressAr || contactInfo.address || (isRTL ? 'غير متوفر' : 'Not available')
