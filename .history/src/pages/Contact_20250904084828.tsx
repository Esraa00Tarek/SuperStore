...
const Contact = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!contactInfo.email) {
      toast({
        ...
        description: isRTL
          ? 'لم يتم تعيين البريد الإلكتروني. يرجى المحاولة لاحقاً.'
          : 'Email is not set. Please try again later.',
        variant: 'destructive',
      });
      return;
    }
    ...
    const mailtoLink = `mailto:${contactInfo.email}?subject=${encodeURIComponent(
      isRTL ? 'رسالة من الموقع' : 'Message from the website'
  ...
  useEffect(() => {
    const { businessHours: fetchedBusinessHours } = getBusinessHours();
    setBusinessHours(fetchedBusinessHours);
  }, []);

  useEffect(() => {
    const unsubscribeContact = settingsService.subscribeToContactInfo((data) => {
      setContactInfo((prev) => ({
        ...prev,
        ...data,
      }));
    ...
    const unsubscribeHours = settingsService.subscribeToBusinessHours((data) => {
      setBusinessHours(data);
    });

    const unsubscribeWhatsApp = settingsService.subscribeToWhatsAppNumbers((data) => {
      setContactInfo((prev) => ({
        ...prev,
        whatsappNumber: data?.products || '',
      }));
    });
    ...
    return () => {
      ...
      if (unsubscribeWhatsApp) unsubscribeWhatsApp();
    };
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50">
      ...
      <div className="max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-8">
        ...
        <div className="text-center mb-8 sm:mb-12">
          <h1 className={`text-2xl sm:text-3xl md:text-4xl font-bold mb-2 sm:mb-4 ${isRTL ? 'arabic-text' : ''}`}>{t('contact.title')}</h1>
          <p className={`text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto ${isRTL ? 'arabic-text' : ''}`}>
            {isRTL 
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-12">
          {/* Contact Form */}
          <Card className="card-shadow w-full bg-white/80 backdrop-blur-sm border-0 shadow-lg">
