...
const Products = () => {
  ...
  const handleWhatsAppOrder = (productName: string, seller: string) => {
    const productsWhatsAppNumber = '962771234567'; // Replace with dynamic fetching logic if needed
    const message = isRTL
      ? `مرحباً، أود طلب ${productName} من ${seller}`
      : `Hello, I would like to order ${productName} from ${seller}`;
    const whatsappUrl = `https://wa.me/${productsWhatsAppNumber}?text=${encodeURIComponent(message)}`;
